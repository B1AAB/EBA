...
public class ChainInformedSampler : ITraversalAlgorithm
{
    ...
    public ChainInformedSampler(Options options, IGraphDb<BitcoinGraph> graphDb, ILogger<BitcoinGraphOrchestrator> logger)
    {
        _options = options;
        _graphDb = graphDb;
        _logger = logger;
    ...
    public async Task SampleAsync(CancellationToken ct)
    {
            ...
            ++attempts <= _options.Bitcoin.GraphSample.MaxAttempts)
        {
            ...
            foreach (var rootNode in rndRootNodes)
            {
                ...
                // TODO: this is using forest fire setting, change
                var graph = await GetNeighborsAsync(
                    rootNodeLabel: ScriptNode.Kind,
                    rootNodeIdProperty: rootNode.GetIdPropertyName(),
                    rootNodeId: rootNode.Id,
                    nodeSamplingCountAtRoot: _options.Bitcoin.GraphSample.PanoramaOptions.NodeSamplingCountAtRoot,
        ...
        CancellationToken ct)
    {
        ...
        var nodesToKeep = nodesUniqueToThisHop.Keys.ToHashSet(); // TODO: so keeping all, if this is the right path, simplify it by removing this type
            /*.OrderBy(
            x => rnd.Next()).Take(
        ...
        {
            if (g.NodeCount >= _options.Bitcoin.GraphSample.MaxNodeCount // TODO: also this setting should be CIS specific and set it to a very high number
                || g.EdgeCount >= _options.Bitcoin.GraphSample.MaxEdgeCount) // TODO: also this setting should be CIS specific and set it to a very high number
            {
        ...
        CancellationToken ct)
    {
        ...
        if (hop < maxHops)
        {
            ...
            foreach (var node in selectedNodes)
            {
                ...
                if ((node.OriginalInDegree != null && node.OriginalInDegree > 1000) ||
                    (node.OriginalOutDegree != null && node.OriginalOutDegree > 1000))
                {
        ...
        CancellationToken ct)
    {
        ...
        foreach (var txNode in g.NodesByType[NodeKind.Tx])
        {
            ...
            if (!found)
            {
                // todo need better property setting
                ...
                g.TryGetOrAddEdge(candidateEdge, out _);
            }
        }
