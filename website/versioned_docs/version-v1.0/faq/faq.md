...
* `httpclientpolicy ... service unavailable`
    ...
    The warning means EBA is making more concurrent RPC calls to `Bitcoin Core` that exceeds its threshold. 
    You can change the threshold by increasing the value of `-rpcworkqueue` when you start `Bitcoin Core`. 
    When this limit is reached, EBA will safely wait and retry a few times after dynamically determined 
    wait intervals (so to lower the load on `Bitcoin Core`). However, if EBA still fails to fetch data 
