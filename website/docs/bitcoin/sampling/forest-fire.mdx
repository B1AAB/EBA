...
| [`NodeSamplingCountAtRoot`](#count-at-root) | `int`    | `100`   | Maximum number of neighbors to sample from the root node. |
...
### `NodeSamplingCountAtRoot` {#count-at-root}
...
```
floor(NodeSamplingCountAtRoot - (hop * reductionFactor))
```
