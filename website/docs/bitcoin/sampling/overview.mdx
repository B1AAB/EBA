...
- [Panorama sampling algorithm](panorama)


Additionally, we provide the following ready-to-use sampled communities:

- `200k` communities sampled using Forest Fire method containing only _Script-to-Script_ edges.
...
-   Run the `sample` method; you may run the following command for a documentation on the command's arguments.
    ...
    ```bash
    .\aab.eba.exe bitcoin sample --help
    ```
...
```bash
.\aab.eba.exe bitcoin sample --status-filename .\my_options.json
```
:::
